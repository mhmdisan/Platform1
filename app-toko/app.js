...
function navigateTo(page, e) {
    ...
    // Page title
    const titles = { daftar: 'Daftar Barang', tambah: 'Tambah Barang', dashboard: 'Dashboard Statistik' };
    ...
    // Muat grafik & ringkasan saat masuk ke halaman dashboard
...
//  Data Labels & Values diambil dari statistik.php
//  (backend sudah melakukan ORDER BY + LIMIT, frontend
//  hanya menggambar — bukan menghitung ulang).
// ══════════════════════════════════════════════
let chartBarangInstance = null;   // instance Chart.js yang sedang aktif
let dashboardLabels = [];         // cache array Labels dari statistik.php
let dashboardValues = [];         // cache array Values dari statistik.php
let chartTypeAktif = 'bar';       // tipe grafik yang sedang ditampilkan

// Palet warna untuk pie/doughnut (tiap barang dapat warna berbeda)
const PALET_CHART = ['#1A56FF', '#059669', '#D97706', '#EF4444', '#8B5CF6', '#06B6D4', '#EC4899', '#84CC16', '#F97316', '#3B82F6'];

function formatRupiah(angka) {
    return 'Rp ' + Number(angka || 0).toLocaleString('id-ID');
}

// 1. Ambil data dari statistik.php
async function loadDashboard() {
    try {
        const response = await fetch(`${API_BASE}statistik.php?limit=5`);
        ...
        // 2. Tangkap 2 array JSON: Labels & Values, lalu simpan ke cache
        dashboardLabels = hasil.labels || [];
        dashboardValues = hasil.values || [];

        // 3. Isi kartu ringkasan
        const r = hasil.ringkasan || {};
        document.getElementById('dash-total-barang').textContent = r.total_barang ?? 0;
        document.getElementById('dash-total-nilai').textContent  = formatRupiah(r.total_nilai);
        document.getElementById('dash-rata-harga').textContent   = formatRupiah(Math.round(r.rata_rata_harga || 0));
        document.getElementById('dash-termahal').textContent     = r.nama_termahal || '-';

        // 4. Lempar data ke dalam new Chart() (tipe yang sedang aktif)
        renderChartBarang(chartTypeAktif);
    } catch (error) {
        showToast('❌ Gagal terhubung ke server saat memuat statistik.', 'error');
    }
}

// 5. Gambar / gambar-ulang grafik dengan tipe tertentu
function renderChartBarang(type) {
    const canvas     = document.getElementById('chartBarang');
    const emptyState = document.getElementById('chart-empty');

    if (!dashboardLabels.length) {
    ...
    // Hancurkan chart sebelumnya dulu supaya tidak menumpuk / bocor memori
    if (chartBarangInstance) {
        chartBarangInstance.destroy();
    }

    const isLingkaran = (type === 'pie' || type === 'doughnut');
    const warna = dashboardLabels.map((_, i) => PALET_CHART[i % PALET_CHART.length]);

    const ctx = canvas.getContext('2d');
    chartBarangInstance = new Chart(ctx, {
        type: type, // <-- ganti 'bar' jadi 'pie' / 'doughnut' / 'line' di sini
        data: {
            labels: dashboardLabels,
            datasets: [{
                label: 'Harga Barang',
                data: dashboardValues,
                backgroundColor: isLingkaran ? warna : 'rgba(26,86,255,0.75)',
                borderColor: isLingkaran ? '#fff' : '#1A56FF',
                borderWidth: isLingkaran ? 2 : 1,
        ...
        options: {
            ...
            plugins: {
                legend: { display: isLingkaran, position: 'bottom' },
                title: {
                    display: true,
                    text: `Top ${dashboardLabels.length} Barang Termahal`,
                    font: { family: 'Inter', size: 13 }
                },
                tooltip: {
                    callbacks: {
                        label: (item) => {
                            const nilai = (typeof item.parsed === 'object' && item.parsed !== null) ? item.parsed.y : item.parsed;
...
// 6. Tombol switcher tipe chart (bar / pie / doughnut / line)
function gantiTipeChart(type, e) {
    if (e) e.preventDefault();
    chartTypeAktif = type;
